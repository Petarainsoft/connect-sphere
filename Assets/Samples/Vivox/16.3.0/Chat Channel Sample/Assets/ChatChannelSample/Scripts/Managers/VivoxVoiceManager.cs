...
public class VivoxVoiceManager : MonoBehaviour
{
    ...
    [Header("lobbyChannel will be used in case room name is empty")]
    ...
    public async Task InitializeAsync(string playerName)
    {
...
#if AUTH_PACKAGE_PRESENT
        Debug.Log("Vivox InitializeAsync step 1");
        if (!CheckManualCredentials())
        {
        ...
        Debug.Log("Vivox InitializeAsync step 4");
    }
    ...
    public string RoomNameOrDefault => !string.IsNullOrEmpty(_roomName) ? _roomName : LobbyChannelName;
}
