...
namespace RobustFSM.Base
{
    public abstract class MonoFSM<OwnerType> : MonoBehaviour, IFSM where OwnerType : MonoBehaviour
    {
        ...
        public Dictionary<Type, MonoState> States
        {
            get => _states;
            set => _states = value;
        }
        ...
        public void AddState<T>() where T : MonoState, new()
        {
            if ( !ContainsState<T>() )
            {
                MonoState monoState = gameObject.GetComponent<T>();
                if ( monoState == null )
                {
                    monoState = gameObject.GetComponentInChildren<T>();
                }
                
                if ( monoState == null )
                {
                    monoState = gameObject.AddComponent<T>();
                }
        ...
        public virtual void Initialize()
        {
            ...
            if ( CurrentState == null )
                throw new Exception("\n" + name +
                                    "Initial state not specified.\tSpecify the initial state inside the AddStates()!!!\n");
